...
The derivation in this section follows the approach given in \cref{Tomas_thesis, Franchi_thesis}.
While the non-linear dynamics can not be described with matrices, it can be described by the transfer map formalism.
In the frame where the one turn map is represented by a pure rotation - in normal forms coordinates - it can be written as \cref{Tomas_thesis}:

\begin{equation}
    \mathcal{M} = e^{:\tilde{h_{1}}:} e^{:\tilde{h_{2}}:} \ldots e^{:\tilde{h_{n}}:} R
...
An excellent approach to normal forms formalism can be found in \cref{Carlier_thesis}.
...
The formal solution to finding the generating function F is given in \cref{Forest_normal_forms} and the explicit expression is obtained in \cref{Tomas_thesis} as
...
They can be expressed according to \cref{eq:f_rdts} \cref{Tomas_thesis, Franchi_thesis}, where \(Q_x\) and \(Q_y\) are the unperturbed tunes.
