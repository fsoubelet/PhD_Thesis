...
Importantly, no involved magnet should see its powering change by more than \(\sim\)\qty{3}{\percent} with the application of these knobs, which allows for respecting the powering limits of individually powered elements.
...
Across the machine the \(\beta\)-beating is brought down to around \(\sim\)\qty{5}{\percent} depending on the beam and plane compared to the nominal scenario, except for the vicinity of the IP where the deviation is wanted.
A \qty{5}{\percent} \(\beta\)-beating across the machine is an acceptable level as it is close to the level of control achieved during normal operation with all corrections trimmed in, as can be seen in \cref{figure:virgin_vs_corrected_lhcb2}.
