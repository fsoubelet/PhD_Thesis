...
\todo{Ask Tobias where he installed the warm skew quad when we calculated the ratios of \(\sqrt{\beta_x \beta_y}\)} term and made a quick calculation of the field necessary in this magnet.

\todo{How much more to include here?}

\todo{Add some content for the options that were quickly discarded (unbalanced squeeze, orbit offset in sextupoles)?}

% \subsubsection*{Bad options}
...
Want to include the following points here:
\begin{enumerate}
    \item Some bla bla on the observed need for correction of local coupling at the IP
    \item Some mention that we have tools to take care but it's a two-fold problem and there was nothing for coupling at IP
    \item We presented in this chapter a new tool to correct for local coupling at the IP and experimental results, say them
    \item Also had a look at backup solutions in case of MQSX failures, as we rely heavily on these correctors for machine safety. We found potential schemes to be able to safely operate at the cost of luminosity, which was quantified and is planned to be measured in \num{2023}.
    \item Implications to other colliders? Cool story bro.
\end{enumerate}
