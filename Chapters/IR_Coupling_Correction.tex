...
Relevant fills can be found in \cref{table:run3_fills}.

As a first step, the validity of the experimental setup was verified by trimming the RWS in the machine and then checking the effect of the optics rematching knobs.
\Cref{figure:ir5_rws_rematching_relative} shows the additional \(\beta\)-beating induced across the machine for beam~\num{1} from applying the RWS in IR\num{5}, before (\textcolor{skyblue}{blue}) and after (\textcolor{scarletred}{red}) the application of the optics re-matching knob.
...
\begin{figure}[!thb]
    \centering
    \includegraphics*[width=\textwidth]{Figures/IR_Coupling_Correction/rematching_knob_efficiency_relative.pdf}
    \caption{The beam~\num{1} additional \(\beta\)-beating observed in the machine from the implementation of the RWS in IR\num{5} at \qty{6.8}{\tera\electronvolt} and \(\beta^{*}=\) \qty{30}{\centi\meter}, using only triplet quadrupoles (\textcolor{skyblue}{blue}) and with the optics re-matching using all quadrupoles in the IR (\textcolor{scarletred}{red}). The highlighted area (\textcolor{mplorange}{orange}) shows where magnetic elements are affected by the knobs.}
    \label{figure:ir5_rws_rematching_relative}
\end{figure}

The measured impact is in agreement with what was expected from earlier simulations, leading to a \num{15}-\qty{25}{\percent} additional \(\beta\)-beating in the machine depending on the observed beam and plane; while the re-matching knob brought this beating back to about \qty{5}{\percent} where it was previously kept thanks to existing corrections.
% This showcases the great efficiency of the optics re-matching knob.
Considering the state of the machine at the time of these measurements, where the \(\beta\)-beating was kept at \qty{5}{\percent} thanks to existing corrections, the optics re-matching after the application of the RWS shows good efficiency.
...
In these figures, simulations for beam~\num{2} suggest that the reproduced global coupling has little effect on the procedure, and the colinearity knob scan could almost be used alone to determine the corrections.
For beam~\num{1} simulations however the impact of the reproduced global coupling appears much more substantial, as was the case in the machine, and these highlight the need to compare the scan measurements to simulations.
...
While slightly different corrections are suggested from independent measurements of beam~\num{1} and beam~\num{2}, it is possible that both values are simultaneously true.
While most of the error contribution is expected to come from the dual-beam triplet quadrupoles and be common to both beams, errors in double aperture magnets Q\num{4} to Q\num{10} would affect each beam individually.
Furthermore, the orbit and hence feed-down but also the \(\beta\) ratio between sources and correctors are different for both beams.
...
\begin{table}[!htb]
    ...
    $\begin{tblr}{colspec={ccc}}
        ...
        \hline
    \end{tblr}$
    \caption{Correction adjustments suggested by Rigid Waist Shift scans, on top of the existing Segment-by-Segment corrections that were in the machine.}
    \label{table:rws_corrections_summary}
\end{table}

The suggested adjustments were applied, and their impact was assessed based on instantaneous luminosity measurements.
\Cref{figure:corrections_trims} shows the trim performed at IR\num{1} and subsequent measured luminosity changes.
...
\todo{Similar trims were done in both IR\num{1} and IR\num{5} at \(\beta^{\ast} = \) \qty{30}{cm} and \(\beta^{\ast} = \) \qty{42}{cm}.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics*[width=\textwidth]{Figures/IR_Coupling_Correction/corrections_trim_ir5.pdf}
...
\begin{figure}[!htb]
    \centering
    \includegraphics*[width=\textwidth]{Figures/IR_Coupling_Correction/corrections_trim_ir5_split.pdf}
    \caption{Zoomed view of the colinearity knob setting (\textcolor{mplblue}{blue}) and observed IP\num{5} instantaneous luminosity change (\textcolor{mplorange}{orange}) at \qty{6.8}{\tera\electronvolt} and \(\beta^{\ast} = \) \qty{30}{cm}. The blue area highlights the trim values suggested by the RWS method, which varies for beam~\num{1} and beam~\num{2}.}
    \label{figure:corrections_trims_ir1_zoomed}
\end{figure}

For each measurement, values around the suggested adjustment were also tested in order to look for the best local setting.
\Cref{table:rws_lumi_gains} gives a summary of the observed luminosity improvements for each performed trim.
...
\todo{It is expected to observe lower gains at a higher \(\beta^{\ast}\) since the \(\sqrt{\beta_x \beta_y}\) term of \cref{equation:deltaqmin_guignard} is substantially lower in the triplets for the less squeezed optics.
It is also expected to notice a lower improvement at the CMS detector based on the numbers in \cref{table:rws_corrections_summary}: a lower suggested adjustment indicates a smaller coupling error remains in IR\num{5} than in IR\num{1}, and the subsequent smaller applied correction recovers less luminosity.}

\todo{The adjustments determined with the RWS have been incorporated into the nominal corrector settings and the LHC now uses the resulting skew quadrupole powerings in normal operation.}

% \Cref{table:run2_vs_sbs_run3_vs_rws_run3_corrections} gives a summary of the final settings and a comparison to their values in previous years.
...
Using \(L^*\) as the length of this drift space, the phase advance from IP point to said quadrupole is:

\begin{equation}
    \begin{aligned}
        \mu &= \int_0^{L^*} \frac{1}{\beta(s)} ds \\
            &= \int_0^{L^*} \frac{\beta^*}{\beta^*{}^2 + s^2} ds \\
            &= \beta^* \left[\frac{1}{\beta^*} \tan^{-1} \left(\frac{s}{\beta^*} \right) \right]_0^{L^*} \\
            &= \tan^{-1} \left(\frac{L^*}{\beta^*} \right)
    \end{aligned}
    \label{equation:collider_drift_space_phase_advance_derivation}
\end{equation}

Then, for traditionally \(L^* \gg \beta^*\) one gets:

\begin{equation}
    \mu \simeq \tan^{-1} \left( \infty \right) \simeq \frac{\pi}{2}
