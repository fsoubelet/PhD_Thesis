...
Applying a Rigid Waist Shift (RWS) to the beam - meaning all four betatron waists moving simultaneously - allows to break the symmetry of the optics in the IR.
...
The knob is designed so that a setting of \num{1} will result in a \qty{0.5}{\percent} change in the triplet knob powering (individual magnet trims are not used), which induces a waist shift of \(\approx\)\qty{43.5}{\centi\meter}.
...
\Cref{figure:rigid_waist_shift_knob_effect_on_waist} shows how applying the knob in a given IR displaces the beam's waists away from the IP location, and shows how the \(\beta\)-functions at the IP location are modified, using the \(\beta^{\ast} =\) \qty{30}{\centi\metre} optics of \num{2022}.
\todo{say how I did, can cite Carlier paper}
One can note that the minima of the parabolas (\textcolor{mplblue}{blue} and \textcolor{mplorange}{orange} curves) are not found exactly at the zero knob setting, because the design optics include a very small waist at IP\num{1} and IP\num{5}.


\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/IR_Coupling_Correction/rigid_waist_shift_waist_effect_combined.pdf}
    \caption{Simulated effect of the designed RWS knob on the \(\beta^{\ast} =\) \qty{30}{\centi\metre} optics. The \textcolor{mplblue}{blue} line represents the waist displacement from the design location and is almost completely linear with the knob setting. The \textcolor{mplorange}{orange} and \textcolor{mplgreen}{green} lines represent the horizontal and vertical \(\beta\)-functions change at the IP point itself as the waist is displaced, respectively.}
    \label{figure:rigid_waist_shift_knob_effect_on_waist}
\end{figure}

In \cref{figure:rigid_waist_shift_knob_effect_on_betas} is shown how the \(\beta\)-functions are affected by the application of an RWS, also simulated with the \(\beta^{\ast} =\) \qty{30}{\centi\metre} optics of \num{2022}.
In this simulation the lattice was sliced to improve the resolution of the data points.
One can observe how the symmetry of the optics in the IR is broken thanks to the knob (full vs dashed lines).

\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/IR_Coupling_Correction/rigid_waist_shift_betas_effect.pdf}
    \caption{Simulated effect of the designed RWS knob on the \(\beta\)-functions around IP\num{1}, with the \(\beta^{\ast} =\) \qty{30}{\centi\metre} collision optics. The \(\beta\)-functions for both the horizontal (\textcolor{mplb}{blue}) and vertical (\textcolor{mplr}{red}) planes are shown for the nominal (full lines) and shifted waists (dashed lines) scenarios.}
    \label{figure:rigid_waist_shift_knob_effect_on_betas}
\end{figure}

\subsubsection*{Optics Impact and Rematching}

Predictably, the application of an RWS as described above has a strong impact on the optics across the machine.
The RWS is also expected to lead to a \numrange[range-phrase = --]{20}{30}\unit{\percent} increase in \(\beta\)-beating in the machine, depending on the observed plane.
This can be observed in \cref{figure:rigid_waist_shift_betabeating}.

% \begin{figure}[!htb]
%     \centering
%     \includegraphics[width=0.99\textwidth]{Figures/IR_Coupling_Correction/}
%     \caption{Simulated effect of the designed RWS knob on the \(\beta\)-functions around IP\num{1}, with the \(\beta^{\ast} =\) \qty{30}{\centi\metre} collision optics. The \(\beta\)-functions for both the horizontal (\textcolor{mplb}{blue}) and vertical (\textcolor{mplr}{red}) planes are shown for the nominal (full lines) and shifted waists (dashed lines) scenarios.}
...
For instance, simulations show .

\todo{Talk about the impact it has on the optics outside the machine, and why it should be rematched.
Talk about the design of rematched knobs (refer to \cref{section:optics_rematching_knobs_lsa}) and show the beta-beating in both cases.}
