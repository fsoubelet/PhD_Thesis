...
\Cref{figure:rigid_waist_shift_knob_effect_on_waist} shows how applying the knob in a given IR displaces the beam's waists away from the IP location, and shows how the \(\beta\)-functions at the IP location are modified, using the \num{2022} \(\beta^{\ast} =\) \qty{30}{\centi\metre} optics.
...
In \cref{figure:rigid_waist_shift_knob_effect_on_betas} is shown how the \(\beta\)-functions are affected by the application of an RWS, also simulated with the \num{2022} \(\beta^{\ast} =\) \qty{30}{\centi\metre} optics.
