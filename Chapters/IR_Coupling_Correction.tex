...
This chapter, which constitutes the core of this thesis, presents a new method that was developed to determine corrections of linear coupling at the \glspl{IP}.
...
For this, in the LHC the local coupling correction is done by measuring the RDTs in the vicinity of the IP and using the MQSX \gls{skew} quadrupole correctors introduced in \cref{subsection:lhc_eirs} and showcased in \cref{figure:ir5_and_around,figure:lhc_ir_corrector_layout} for correction.
...
A candidate for a better observable that was considered are the \concept{combined coupling RDTs}~\cite{PRAB:Franchi:First_Simultaneous}, or simply Combined RDTs (CRDTs), \(|\hat{F}_{XY}|\) and \(|\hat{F}_{YX}|\).
...
The \concept{colinearity knob} is a powering setting convention for the IR skew quadrupole correctors, the MQSX magnets.
...
\todo{Above listing is "a bit hand wavy" according to Tobias, go over again.}
...
\todo{Here I can mention back to the expectation vs measured from earlier, or maybe I keep that for later?}

\begin{figure}[!htb]
    \centering
    \includegraphics*[width=\textwidth]{Figures/IR_Coupling_Correction/ir1_scan_vs_simu.pdf}
    \caption{Luminosity drop from correction carry-over to the right MQSX at IR\num{1} at \(\beta^{\ast} =\) \qty{30}{\centi\meter}. \todo{Maybe change the scale and start at optimal? Or specify what is operational point on figure?}}
...
Simulations systematically over-estimate the loss of luminosity but some discrepancy is to be expected as \cref{equation:luminosity_double_beams} is a simplified calculation.  % which does not take into account \textit{e.g.} the crossing angles.
...
\begin{table}[!htb]
    ...
    \begin{tblr}{colspec={cccc}}
        ...
        \hline[dashed]
    \end{tblr}
    \caption{Necessary tilt angles of either Q\num{2} or Q\num{3} triplet elements to compensate for the loss of the closest skew quadrupole corrector/ \todo{Mention I show results for IR\num{2} and IR\num{8} but it only really matters for IR\num{1} and IR\num{5}}.}
...
\todo{This is still under investigation so we don't need details, just a mention would be enough. At the moment the idea is to move D1 a bit of get the warm skewquad in IP side.}
