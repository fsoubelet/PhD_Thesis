...
\todo{Do we talk about the lumi imbalance between ATLAS and CMS? It was improved to about 3\% after the adjustment according to Tobias pres. in LNO on Aug 17.}
...
\todo{See with Tobias if he has any more clues to justify the IP1 discrepancies.}
...
\todo{There is probably a bit more to say here, discuss with Tobias.}
\newpage
...
\Cref{table:correctors_peak_dose}, reproduced from~\cite{Evian21:Cerutti:TripletLifetime}, shows the expected total received dose for different magnets in the main IRs, for various scenarios.
...
For this, several containment scenarios were investigated that would try and compensate for corrector failures.
...
Should the most powered correctors fail, the instantaneous luminosity at the affected IP is expected to drop considerably at \(\beta^{\ast} =\) \qty{30}{\centi\meter}: by up to \(\approx\) \qty{60}{\percent}.
...
\todo{Simulations and measurements show a reasonable agreement in both cases.
...
\begin{figure}[!htb]
    ...
    \caption{Integrated luminosity loss over a day for each missing MQSX and for two different baselines of instantaneous luminosity.}
...
% \todo{Mention that this is the most viable option, and we are planning an MD on this?}

\subsection{Operating with Two Missing MQSX}

There is a possibility that \textit{both} MQSXs in a given IR fail.
In this case, the compensation of a corrector's contribution by its counterpart is no longer possible, and the uncompensated global coupling coming from the IR would be too large at small \(\beta^{\ast}\) for safe machine operation.
Simulation results suggest that it would not be feasible to squeeze to the optics below \(\beta^{\ast} =\) \qty{90}{\centi\meter}.

\subsubsection*{Tilt of Triplet Elements}

A potential solution to mitigate the impact of a corrector failure is to tilt a close triplet quadrupole,introducing a skew component in the element's field and relying on this contribution to perform the correction in place of the skew quadrupole corrector.
...
\begin{table}[!htb]
    ...
    \begin{tblr}{colspec={cccc}}
        ...
        \hline[dashed]
    \end{tblr}
    \caption{Necessary tilt angles of either Q\num{2} or Q\num{3} triplet elements to compensate for the loss of the closest skew quadrupole corrector, in IR\num{1} and IR\num{5}.}
...
It is worth noting that tilting a triplet quadrupole has a strong impact on the optics due to the high \(\beta\)-functions at the elements.
...
% \subsubsection*{Adapting the Optics Squeezing Scheme}
