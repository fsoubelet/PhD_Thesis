...
As a consequence, the reconstruction of coupling RDTs close to the IPs is not accurate.
...
Not only can large error bars can be noticed, but no given case appears specifically better than the other while the orange line corresponds to a better correction.
...
A candidate for a better observable would be \intro{combined coupling RDTs}~\cite{PRAB:Franchi:First_Simultaneous}.
These can be expressed from the coupling RDTs as:
...
\todo{UNFORTUNATELY ITS TRASH AND SHOW A PLOT}
...
\subsection{The Colinearity Knob}
