...
So far positive settings of RWS, aka as defined in \cref{table:rigid_waist_shift_knob}.
As explained in \cref{appendix:experimental_knobs} it can be trimmed with a factor 1 but also -1, which requires a different rematching knob etc.
Measurements were done in both configurations, and the results show above are with a factor 1.
It so happens that all results from measurements with a factor -1, aka negative rws, led to retarded and un-physical corrections and were ignored.
Include figures for each measurement.

Cite the issues that we had with the Q6 and not the full trims when doing these measurements.
Mention that the data from each measurement was looked into and no particular issue was found, we don't know why it was shit.
