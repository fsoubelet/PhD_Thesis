...
\begin{figure}[!htb]
    \centering
    \includegraphics*[width=\columnwidth]{Figures/IR_Coupling_Correction/crdt_fxy_nominal_vs_colin_md_2018.pdf}
    \caption{Reconstructed CRDT \(|\hat{F}_{XY}|\) around IP\num{2} from measurements at \(\beta^{\ast} =\)~\qty{50}{\centi\meter} during a \num{2018} MD. The \textcolor{mplblue}{blue} lines correspond to \num{2} measurements with the nominal optics, and \textcolor{mplorange}{orange} lines correspond to \num{3} different measurements with a local coupling bump implemented around IP\num{2}. The percentages indicate the strength of the AC dipole kicks.}
    \label{figure:crdt_fxy_nominal_vs_colin_md_2018}
\end{figure}

\todo{Comment on the results here, we don't have the error bars like in \cref{figure:beamtest_vs_2022_sbs_abs_f1001_ir1}, but the reproducibility is not good, and quite different results from various measurements (different kick strengths).}
...
\begin{itemize}
    ...
    \item Do a brief kind of conclusion so far saying we need 2 new things: a way to adjust for the coupling at the IP and a good observable to rely on to drive the correction
...
Since for round optics in the LHC the \(\beta\)-functions are by design identical at the MQSX position left and right of the IP (see \cref{figure:lhc_ir5_zoomed}), and the phase advance between the two magnets is \(\pi\).
Assuming a good \(\beta\)-beating correction, the knob induces a closed coupling bump going from corrector to corrector around the IP without impacting the machine's global coupling.
