...
In order to achieve a small \(\beta^{\ast}\) at the IPs, the beams are focused using a \intro{triplet} of quadrupoles just before the IP.
...
Other IRs, not of interest to this thesis, have significantly different layout which can be seen in details in~\cite{BOOK:Bruning:LHC_Design_Report_Main_Ring,PHD:Vanbavinckhove}.

\subsection{Matching Sections and Dispersion Suppressors}
\label{subsection:matching_sections_dispersion_suppressors}

Assuring the transition between the arcs and the specific optics conditions of the IRs are \intro{matching sections}, as can be seen on \cref{figure:ir5_and_around}.
On the figure, the area designated as matching section also includes the \intro{dispersion suppressor}.
Together, the two segments are responsible for matching the TWISS functions between the arcs and the IRs, and to reduce the dispersion to near-zero value, respectively.
One of the dispersion suppressor leading to IP\num{5} can be seen on \cref{figure:lhc_dispersion_suppressor}, where the beam travels from left to right.
...
The full matching section, from the start of the dispersion suppressor to just before separation dipole D\num{2}, is shown in \cref{figure:lhc_matching_section}.
The quadrupoles Q\num{4} to Q\num{7} are matching quadrupoles and transition the TWISS functions from their values in the arcs to that at the entrance of the triplets.
Quadrupoles Q\num{4} to Q\num{10} are individually powered magnets.
