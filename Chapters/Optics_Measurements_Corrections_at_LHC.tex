...
The beams occupy separate apertures, or beam pipes, except in the \IRs where they are made to collide.
...
The ratio between the peak \(\beta\)-functions induced in the arcs is called \intro{telescopic index}, or tele-index, and is denoted \(r_{Tele}\)~\cite{CERN:Fartoukh:Round_Telescopic_Optics_LHC_Large_Telescopic_Index}.
It is defined as:
...
The resulting configuration, called \intro{stable beams}, is kept throughout the luminosity production for the fill.
...
In \num{2022} a \intro{\(\beta^{\ast}\)-leveling} was introduced, were collisions start at a \(\beta^{\ast} =\)~\qty{60}{\centi\meter} and the \(\beta^{\ast}\) is progressively reduced to \qty{30}{\centi\meter} during stable beams.
...
In the LHC, the \intro{Base Band Tune} (BBQ) system~\cite{CERN:Boccardi:LHC_Transverse_Diagnostics_Systems,CERN:Boccardi:LHC_BBQ_Tune_Chromaticity_Systems} provides continuous, passive monitoring of the tune by performing spectral analysis of the turn-by-turn data at a specific location in IR\num{4}.
...
As a first step the recorded raw turn-by-turn data is cleaned of noise using a \intro{Singular Value Decomposition} (SVD) approach~\cite{PRAB:Calaga:Statistical_Analysis_RHIC_BPMs}.
...
It is possible to directly measure the \(\beta^{\ast}\) at the IPs without beam excitation using the \intro{\(k\)-modulation} technique~\cite{IPAC:Calaga:LHC_K_Modulation}.
...
In the LHC local optics errors are determined and corrected using the \intro{Segment-by-Segment} (SbS) technique~\cite{PRAB:Tomas:CERN_LHC_OMC,PRAB:Tomas:Review_Linear_Optics_Measurements}.
