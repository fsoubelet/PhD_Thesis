...
The \gls{LHC} is a \qty{26.659}{\kilo\metre} long synchrotron collider located at the \gls{CERN}, on the French-Swiss border.
...
The beams occupy separate apertures - or beam pipes - side by side except in the \glspl{IR} where they are made to collide.
...
Each cell contains two \concept{MQ} (one MQF, one MQD) with three MBs in between, for a total of 6 MBs per cell.
...
Part of the main assemblies are superconducting \concept{spool piece magnets}, correctors used for the local compensation of magnetic errors in the main arc magnets~\cite{BOOK:Bruning:LHC_Design_Report_Main_Ring}.
...
In the layout (top) part of the plot the powering of elements is indicated, with MBs in \textcolor{mplblue}{blue}, MQs in \textcolor{latwiss_red}{red}, MSs in \textcolor{latwiss_yellow}{yellow} and MOs in \textcolor{latwiss_green}{green}.
Beam position monitors (BPMs) are indicated as grey patches.
Note that not all elements are indicated.
...
The purpose of each straight section is briefly stated in \cref{figure:lhc_schematic_layout} and detailed in \cref{table:lhc_straight_sections}.
\\
...
In order to achieve a small \(\beta^{\ast}\) at the IPs, the beams are focused using a superconducting \concept{triplet} of quadrupoles, on either side of and close to the IP~\cite{CERN:Ostojic:Improved_Optical_System_LHC_Triplet}.
...
Other IRs, of lower interest to this thesis, have significantly different layouts which can be found in detail in~\cite{BOOK:Bruning:LHC_Design_Report_Main_Ring,PHD:Vanbavinckhove}.
...
The \gls{ATS} optics scheme~\cite{CERN:Fartoukh:ATS_Report,PRAB:Fartoukh:Achromatic_Telescopic_Squeeze,IPAC:Pojer:LHC_ATS_Experience} consists of splitting the reduction of the \glssymbol{betastar} - the squeeze - into two stages.
...
In this stage, the \concept{tele-squeeze}, the \(\beta^{\ast}\) is reduced by using the matching quadrupoles in the adjacent IRs: IR\num{2} and IR\num{8} for the tele-squeeze of IR\num{1}, and IR\num{4} and IR\num{6} for the tele-squeeze of IR\num{5}.
...
The ratio between the peak \(\beta\)-functions induced in the arcs is called the \concept{telescopic index}, or tele-index, and is denoted \(r_{Tele}\)~\cite{CERN:Fartoukh:Round_Telescopic_Optics_LHC_Large_Telescopic_Index}.
It is defined as:
...
This ATS optics scheme has been used in the LHC since \Gls{run}~\num{2} and allowed the reduction of the collision optics \glssymbol{betastar} from its design value of \qty{55}{\centi\meter} to \qty{30}{\centi\meter}.
It is the operational baseline for Run~\num{3}.
...
For luminosity production a larger number of high intensity bunches is injected: of the order of a few \num{e3} bunches, with \(\ge\) \num{e11} protons per bunch.
...
This last setting may be changed by machine operators during stable beams in order to optimize the beam lifetime.

Starting in \Gls{run}~\num{3}, some additional complexities were added to the cycle that are not shown in \cref{figure:lhc_cycle}.
In \num{2022} a \concept{\(\beta^{\ast}\)-leveling} was introduced, where collisions start at \(\beta^{\ast} =\)~\qty{60}{\centi\meter} and the \glssymbol{betastar} is progressively reduced to \qty{30}{\centi\meter} during stable beams.
...
This \(\beta^{\ast}\)-leveling will start at \(\beta^{\ast} =\)~\qty{1.2}{\meter} in \num{2023} and \num{2024}, with a higher pile-up value.
...
One can then define the \gls{beta-beating}, a good indicator of the quality of the linear \gls{optics}, as the relative deviation of the machine's \(\beta\)-functions from that of the design values.
It is defined as:
...
From these, comparisons to model values are made which allow for an assessment and understanding of the errors in the machine; and corrections can be calculated and applied to bring the optics as close to the nominal scenario as possible.
...
The tune and aperture kickers~\cite{CERN:Barlow:Control_MKQA_LHC,IPMS:Carlier:Kicker_Pulse_Generator_Measurement_Tune_Dynamic_Aperture_LHC} operate as traditional kicker magnets: ramping their magnetic field up and down in a single turn, applying a transverse kick to the beam and then allowing free motion.
...
For \gls{optics} measurements lasting oscillations are preferred, as they increase the spectral resolution and reduce the noise floor in the spectral analysis of turn-by-turn data.
...
Such a non-destructive, sustained excitation of the beams can be achieved with an \gls{AC_dipole}.
It is a rapidly oscillating dipole magnet which can generate forced driven oscillations with large amplitudes by exciting the beam at frequencies close to the natural tunes~\cite{PHD:Miyamoto,CERN:Serrano:LHC_ACDipole_Introduction}.
Moreover, the AC dipole strength can be ramped up and down adiabatically~\cite{PRAB:Tomas:Adiabaticity_Ramping_Process_AC_Dipole} and kept constant at high amplitudes, allowing for long lasting coherent oscillations of the beam.
...
It is important to note that the forced oscillations introduce a perturbation on the optics.
...
When the AC dipole is driving the beam, an equivalent parametrization exists, and denoting the transverse driven coordinate as \(z_D(s)\) one can express it as~\cite{PHD:Miyamoto,PRAB:Tomas:Normal_Form_Particle_Motion_AC_Dipole}:
...
The form of \cref{equation:driven_transverse_motion} makes the assumption that the forced oscillation term depends only on \(\phi_D\).

The \(\beta\)-function modified by the AC dipole, corresponding to the perturbed optics under forced oscillations, is given by~\cite{PRAB:Miyamoto:Parametrization_Driven_Betatron_Oscillation}:
...
\end{equation}
where \(\delta_D = Q_D - Q_z\) corresponds to the aforementioned gap, often called the tune delta.
...
This method has also been extended to use specifically chosen combinations of \(N\) BPMs~\cite{PRAB:Langner:N_BPM_Method,PRAB:Wegscheider:Analytical_N_BPM_Method}, which improves its precision as it then only depends on the measured phase advances and is fully independent of BPM calibration~\cite{PRAB:Langner:Optics_Measurement_Algorithms_Error_Analysis_Proton_Energy_Frontier}.
...
By modulating both innermost quadrupoles near the IP, one determines the average \(\beta\)-functions in these quadrupoles, which can then be propagated through the drift space in between to determine both the \(\beta^{\ast}\) value as well as any potential waist shift~\cite{PRAB:Carlier:KModulation_HiLumi}.
...
Mentioned in \cref{subsection:optics_measurements} is the effect of \gls{AC_dipole} excitation on the observables, which differ from those of the natural machine.
...
In this case, where only the position data is used, the spectal analysis will mix up lines \(Z(a,b)\) and \(Z(-a, -b)\) as it cannot distinguish one from the other.
...
It may, however, be possible to neglect one of them depending on their relative strengths.
...
Inverting the settings found and applying the inverted values in the machine corrects the measured errors.

This method is mostly used in the LHC \glspl{IR}, where the \(\beta\)-beating is corrected by compensating the discrepancies in the betatron phase, which has the same impact as correcting the \(\beta\)-beating directly but proved to be a more precise and local observable~\cite{PRAB:Tomas:CERN_LHC_OMC}.
...
\Cref{figure:example_sbs_correction_coupling} shows an example of a local correction of the coupling RDTs around IP\num{2} from the LHC \num{2021} beam test.
...
\begin{equation}
  \mathbf{R} \Delta \vec{K} = \left(\Delta \overrightarrow{\phi_x}, \Delta \overrightarrow{\phi_y}, \frac{\Delta \overrightarrow{\beta_x}}{\beta_x}, \frac{\Delta \overrightarrow{\beta_y}}{\beta_y}, \Delta \frac{\overrightarrow{D_x}}{\sqrt{\beta_x}}, \Delta Q_x, \Delta Q_y \right) \text{ .}
...
To calculate corrections the pseudo-inverse of the \(\mathbf{R}\) matrix, denoted \(\mathbf{R^{-1}}\), is calculated and multiplied with the measured deviations from the model.
...
The \(w_1 \ldots w_7\) terms are weights which can be adjusted to either focus on correcting a given quantity, to ignore some parameters completely or to balance the corrections between all properties.
