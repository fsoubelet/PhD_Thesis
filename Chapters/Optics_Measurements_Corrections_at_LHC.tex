...
\todo{Mention this driving of the motion affects it and we need to take it into consideration, will be talked about later?}
...
\begin{equation}
  \lambda_D = \frac{\sin \left(\pi Q_{-}\right)}{\sin \left(2 \pi Q_x + \pi Q_{-}\right)} \text{ ,}
  \label{equation:driven_oscillations_lambda_D}
\end{equation}
in which \(Q_{\pm} = Q_D \pm Q_x\) corresponds to the tune gap.

In the LHC the AC Dipole is usually driven with a horizontal tune delta \(Q_{x,-}\) of \num{-0.01} and a vertical one \(Q_{y,-}\) of \num{0.012}.
One can see in \cref{figure:ac_dipole_induced_beta_beating} the impact of an AC Dipole on the vertical \(\beta\)-function in a simple FODO lattice, where the difference between the free and forced cases is clearly visible.
...
One can then express the \(\beta\)-beating induced by the AC Dipole, as defined in \cref{equation:beta_beating_definition}.
From \cref{equation:driven_beta_function} and \cref{equation:driven_oscillations_lambda_D} one can see express this beating according to:
...
\begin{figure}[!htb]
  ...
  \caption{AC Dipole induced \(\beta\)-beating in a general case, for various phase advances between an element where the observation is made and the AC Dipole.}
...
This impact of the AC Dipole on the optics is taken into account and compensated in analysis steps.

\subsubsection*{Spectral Analysis}

As a first step the recorded raw turn-by-turn data is cleaned using a \intro{Single Value Decomposition} (SVD) approach~\cite{PRAB:Calaga:Statistical_Analysis_RHIC_BPMs}.
...
\begin{equation}
  \beta^i_z = \frac{\cot \left(\phi_z(i \rightarrow j)\right) + \cot \left(\phi_z(i \rightarrow k)\right)}{\frac{M_{11}(i \rightarrow j)}{M_{12}(i \rightarrow j)} + \frac{M_{11}(i \rightarrow k)}{M_{12}(i \rightarrow k)}}  \text{ ,} \quad z = x, y \text{ ,}
  \label{equation:beta_from_phase}
\end{equation}
where the \(M_{ab}(i \rightarrow j)\) are elements of the transfer matrix between BPMs \(i\) and \(j\), assumed to be that of the model.
...
From \cref{equation:single_particle_action}, one infers that the \(\beta\)-function can also be determined directly from the amplitude of the oscillations recorded at BPMs.
...
\begin{equation}
  \begin{aligned}
    \beta_z^{\mathrm{amp}}(i) &= \frac{A_z^2(i)}{2 J_z} \text{ ,} \\
    % 2 J_z                     = \frac{\sum_n \frac{(\mathrm{peak-to-peak} / 2)^2}{\beta_z^{\mathrm{model}}}}{n} \text{ .}
...
\begin{figure}[!htb]
  ...
  \caption{The \(\beta\)-beatings obtained when reconstructing the \(\beta\)-functions from phase (\textcolor{mplblue}{blue}) and from amplitude (\textcolor{mplorange}{orange}).}
...
This method is mostly used in the LHC IRs, where the the \(\beta\)-beating is corrected by correcting the discrepancies in the betatron phase, which has the same impact as correcting the \(\beta\)-beating directly~\cite{PRAB:Tomas:CERN_LHC_OMC}.
...
\begin{figure}[!hbt]
  ...
  \caption{Local phase correction in IR\num{5} from the LHC \num{2022} commissioning at flat-top. The vertical line indicates the position of IP\num{5} in the segment. The \textcolor{mplblue}{blue} line shows the phase deviation from the model values, while the \textcolor{mplorange}{orange} line shows the effect of the reconstructed errors on the model.}
