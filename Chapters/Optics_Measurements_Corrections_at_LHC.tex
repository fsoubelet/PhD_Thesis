...
\Cref{figure:lhc_crossing_schemes_ip15} shows the crossing schemes for both IR\num{1} and IR\num{5} for the \(\beta^{\ast} =\) \qty{30}{\centi\metre} collision optics, with the location of the triplets highlighted in gray and that of the separation dipoles in yellow.
...
When pushing the \glssymbol{betastar} to smaller values, and therefore the \(\beta\)-functions in the triplets to higher ones, the chromatic effects produced by the triplet quadrupoles~\cref{equation:natural_chromaticity_approximation} increase drastically and need to be corrected.
