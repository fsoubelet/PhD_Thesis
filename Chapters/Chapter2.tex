...
Starting from the Hamiltoninan Equations

\begin{equation}
    \dfrac{d \vec{p_z}}{d t} = - \frac{\partial \mathcal{H}}{\partial \vec{z}} \quad \dfrac{d \vec{z}}{d t} = \frac{\partial \mathcal{H}}{\partial \vec{p_z}} \text{ ,}
    \label{equation:hamiltonian_equations}
\end{equation}

where the Hamiltonian for the transverse planes for a multipole of order n is given by~\cite{PHD:Tomas, PHD:Franchi}
...
In the linear regime, it follows that
...
The contribution to the Hamiltonian of a multipole of order \(n\) is given by~\cite{PHD:Tomas}:

\begin{equation}
    \mathcal{H_n} = \frac{q}{p} \operatorname{Re} \left[ \frac{1}{n} \left[ B_n(s) + i A_n(s) \right](x + i y)^n \right] \text{ .}
    \label{equation:hamiltonian_multipole_order_n}
\end{equation}
