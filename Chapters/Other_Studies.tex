...
Assuming the normal nature of \gls{BPM} measurements, one can make use of the underlying properties of the chi-square and chi statistical distributions in order to compute the standard deviation of phase advances while also differentiating between categories of BPMs, without performing measurements involving only BPMs of interest.
...
\todo{Maybe one could measure a delta meaning that one change strength somewhere and see the difference.}
\todo{Also the conclusion should be that it has been demonstrated.}
...
Even if they don't have a large effect normally, if there had been a different scaling with the AC-dipole maybe they would?
Now we know that they don't, that is also a result.