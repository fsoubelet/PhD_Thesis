...
As a convention, a quadrupole focusing in the horizontal transverse plane and defocusing in the vertical one is referred to as a \intro{focusing quadrupole}. 
...
The \(\alpha_u (s)\), \(\beta_u (s)\), \(\gamma_u (s)\) and \(\phi_u (s)\) are also called the \intro{Twiss parameters}~\cite{RSI:Twiss:Orbital_Stability_Proton_Synchrotron}.
...
% The same transformation is valid for \((u, p_u)\) to \((\hat{u}, \hat{p}_u)\) coordinates.
...
For a particle with a momentum \(p \neq p_{0}\) one defines and uses the \intro{relative momentum deviation} \(\delta\):
...
This is illustrated in \cref{figure:phase_space_non-linear_physical_normalized_normal_form_coordinates}.
