...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.9\linewidth]{Figures/Beam_Dynamics_Theory/design_vs_closed_vs_particle_orbit.pdf}
...
For each magnet applying a field \(B\) one can define the \gls{beam-rigidity}, which is an indication of the field's ability to alter a particle's course based on its charge \(q\) and momentum \(p\), as:
...
\begin{noteblock}
    ...
    From now on, for clarity of the exposed expressions \(z\) will be used to denote either \(x\) or \(y\).
\end{noteblock}
...
\(\phi_z(s)\) is the \concept{phase advance} and \(\alpha_z(s)\) is the \concept{alpha-function}.
\(\beta_z(s)\) is the \concept{beta-function} and represents beam envelope, or size, around the ring: it describes the transverse position dependent amplitude of the oscillation and has the dimension of a length.
...
\todo{Talk about the fact that this is 2D, and we can switch between the appropriate matrices depending on the plane we consider (a foc. quad. in horizontal is def. in vertical so for the x coordinate we use 2.10 and for the y coordinate we use 2.11).
We can make one big 4D matrix which is the inclusion of both (see below: Each 4x4 transfer matrix consists of two 2x2 matrices Mx, My parametrizing the transformation of the coordinates in the horizontal and the vertical planes)}
\begin{equation}
    \mathbf{M}=\left(\begin{array}{cc}
    \mathbf{M}_x & \left(\begin{array}{ll}
    0 & 0 \\
    0 & 0
    \end{array}\right) \\
    \left(\begin{array}{ll}
    0 & 0 \\
    0 & 0
    \end{array}\right) & \mathbf{M}_y
    \end{array}\right)
    \label{equation:4d_transfer_matrix}
\end{equation}

\todo{Then introduce the fact that on the other diagonal it is only 0 if we have uncoupled motion. In case it is coupled, for example, with a skew quadrupole, the terms are nonzero then give the transfer matrix of a skew quadrupole (but we will talk about this later on)}

\begin{equation}
    \left(\begin{array}{cccccc}
    \frac{1}{2}(\cos \sqrt{k_{1s}} L+\cosh \sqrt{k_{1s}} L) & \frac{1}{2 \sqrt{k_{1s}}}(\sin \sqrt{k_{1s}} L+\sinh \sqrt{k_{1s}} L) & \frac{1}{2}(\cos \sqrt{k_{1s}} L-\cosh \sqrt{k_{1s}} L) & \frac{1}{2 \sqrt{k_{1s}}}(\sin \sqrt{k_{1s}} L-\sinh \sqrt{k_{1s}} L)  \\
    -\frac{1}{2} \sqrt{k_{1s}}(\sin \sqrt{k_{1s}} L-\sinh \sqrt{k_{1s}} L) & \frac{1}{2}(\cos \sqrt{k_{1s}} L+\cosh \sqrt{k_{1s}} L) & -\frac{1}{2} \sqrt{k_{1s}}(\sin \sqrt{k_{1s}} L+\sinh \sqrt{k_{1s}} L) & \frac{1}{2}(\cos \sqrt{k_{1s}} L-\cosh \sqrt{k_{1s}} L)  \\
    \frac{1}{2}(\cos \sqrt{k_{1s}} L-\cosh \sqrt{k_{1s}} L) & \frac{1}{2 \sqrt{k_{1s}}}(\sin \sqrt{k_{1s}} L-\sinh \sqrt{k_{1s}} L) & \frac{1}{2}(\cos \sqrt{k_{1s}} L+\cosh \sqrt{k_{1s}} L) & \frac{1}{2 \sqrt{k_{1s}}}(\sin \sqrt{k_{1s}} L+\sinh \sqrt{k_{1s}} L) \\
    -\frac{\sqrt{k_{1s}}}{2}(\sin \sqrt{k_{1s}} L+\sinh \sqrt{k_{1s}} L) & \frac{1}{2}(\cos \sqrt{k_{1s}} L-\cosh \sqrt{k_{1s}} L) & -\frac{\sqrt{k_{1s}}}{2}(\sin \sqrt{k_{1s}} L-\sinh \sqrt{k_{1s}} L) & \frac{1}{2}(\cos \sqrt{k_{1s}} L+\cosh \sqrt{k_{1s}} L) \\
    \end{array}\right)
...
Similarly to the \(\beta\)-function, the \concept{gamma-function} \(\gamma_u(s)\) describes the envelope of oscillations in \(x^{\prime}\) and \(y^{\prime}\).
...
The geometric emittance \(\varepsilon_u\) introduced in \cref{equation:hill_solution}, also named the \concept{Courant-Snyder invariant}, defines together with the \gls{Twiss_parameters} \(\alpha_z(s)\), \(\beta_z(s)\) and \(\gamma_z(s)\) the equation of the phase space ellipse:
...
The deviation of an off-momentum particle orbit from that of the synchronous particle is defined by the \concept{dispersion function} \(D(s)\).
...
\begin{figure}[!htb]
    ...
    \caption{Phase space in normalized coordinates from tracking \num{200} particles in a simple FODO-based lattice, when exciting a third order resonance. Points of each color corresponds to the trajectory of a given particle. \todo{maybe do one where we've got the coupling resonance.}}
...
The transformation to the \concept{normal form coordinates} \(\left( \zeta_x^{+}, \zeta_x^{-}, \zeta_y^{+}, \zeta_y^{-} \right)\) is expressed as:
...
While one can calculate the evolution of the \glsuseri{courantsnyderparameter} coordinates by applying the map \( \mathcal{M} \), the approach is complicated to solve.
...
%     \caption{\todo{Include a plot of a spectrum from an LHC measurement, something out of omc3?}}
...
It can be described with a Taylor expansion of the tune \(Q_z\) around the unperturbed tune as:
...
When these motions share a dependency, they are said to be \concept{coupled}, and one refers to this phenomenon as \gls{betatron_coupling}.
...
The relation between the \concept{unperturbed tunes} \(Q_{x,y}\) and the \concept{coupled tunes} \(Q_{1,2}\) is given by~\cite{CAS:Bryant:Theory_Weak_Betatron_Coupling}:
...
\(S\) is the \gls{luminosity} reduction factor and depends on various parameters such as crossing angles, orbit offsets, beam-beam interactions, beam tilts in the case of \gls{flat_optics} etc.
...
The downside to small beam sizes, which is a direct consequence of the \concept{Louiville theorem}, is that they cause large beam divergence.
