...
\begin{figure}[!htb]
    ...
    \caption{Illustration of a design closed orbit (\textcolor{mplblue}{blue}) and a particle orbit oscillating around it (\textcolor{red}{red}). \todo{Not exactly!}}
...
Using \cref{equation:normal_form_N_turn_expression} and \cref{equation:inverse_transformation_normal_form_to_courant_snyder_coordinates}, the linearly normalized coordinates can be expressed after \(N\) turns as~\cite{CERN:Bartolini:Normal_Form_Tracking_Beam_Data}:
...
The Edwards-Teng parameterization is used in the \gls{MAD} code MAD-X~\cite{CODE:MADX_guide} when handling coupled motion.
In \gls{MAD}-X the relevant parameters are \(\alpha_{x, y}\), \(\beta_{x, y}\), \(\mu_{x, y}\), \(\gamma_{x, y}\) and \(r_{11}\), \(r_{12}\), \(r_{21}\), \(r_{22}\), where \(r_{11} \ldots r_{22}\) correspond to the elements of \(\mathbf{K}\) multiplied by \(\tan(\theta)\).
...
\todo{I should mention how this is affected by the coupling, could talk about it here? What matters is overlap of integrals, if there is an increase from coupling then we would lose luminosity.}
...
\todo{Irrelevant, should remove this. Say a bit of: here are some things that influence S (beam size, crossing angles, some tilt if you've got flat beams, etc)}
