...
Betatron coupling will be discussed in more detail later on, and for now only the \num{2}D case of a given plane will be considered.
...
\end{equation}
where \(\mathrm{M_i}\) is the transfer matrix of the \(i^{\mathrm{th}}\) in the machine.
...
Such momentum offsets introduce \concept{chromatic errors} in the beam dynamics.
...
\begin{equation}
    \left(j-k\right) Q_x + \left(l-m\right) Q_y = p \text{ ,} \quad \quad \text { where } j, k, l, m, p \in \mathcal{Z} \text{ .}
...
Therefore the \(f_{jklm}\) terms are called \acrlongpl{RDT}.%\glspl{RDT}.

For this reason, the tune is one of the single most important design parameters in synchrotrons.
The chosen operational transverse tunes of a synchrotron are known as its \concept{working point}, and should be selected carefully in order to avoid resonances.
...
The \glspl{RDT} vary in amplitude through the machine as they depend on the local multipole strength of contributing sources.
...
For coupled motion a dependency between the horizontal and vertical planes is introduced, and as mentioned in \cref{subsection:equations_of_motion_and_twiss_parameters} the transverse motion can no longer be described by two independent \(2 \times 2\) matrices.
...
Through \(\Sigma\) the coupling can be characterized by the values of the cross-plane elements \(\left\langle x y \right\rangle\), \(\left\langle x p_y \right\rangle\), \(\left\langle p_x y \right\rangle\) and \(\left\langle p_x p_y \right\rangle\).
...
In the Mais-Ripken parameterization, the Ripken parameters are constructed in order to be related to the \(\Sigma\) matrix in a similar way to that shown in \cref{equation:2d_sigma_matrix_to_twiss_parameters} for the \num{2}D uncoupled case.

\subsection{Coupled Motion}
\label{subsection:coupled_motion}

To the first order, according to \cref{equation:resonance_condition}, linear coupling drives the two resonances \(Q_x + Q_y = p\) and \(Q_x - Q_y = p\), with \(p \in \mathcal{Z}\).
...
When \(\Delta \gg \abs{C^{-}}\), away from the resonance, the observed oscillation modes \((Q_1, Q_2)\) are almost identical to the uncoupled tunes \((Q_x, Q_y)\).
When the tunes are moved closer together, approaching the difference resonance (\(\Delta \rightarrow 0\)), the perturbed tunes are forced apart by the coupling and a minimal tune separation \(\Delta Q_{\mathrm{min}}\) can be observed.
The \glssymbol{Cminus} of \cref{equation:unperturbed_tunes_to_coupled_tunes}, named the \gls{Cminus}, corresponds to this separation.
...
Since they describe the same dynamics it is customary to label both of them as \(f_{1001}\), which is a convention used throughout this document.
...
\end{equation}
where \(k_l\) is the \(l^{\mathrm{th}}\) integrated \gls{skew} quadrupole strength, \(\beta_{x,y}^{l}\) are the \glspl{beta-function} at the location of the \(l^{\mathrm{th}}\) skew quadrupole, \(Q_{x,y}\) are the horizontal and vertical tunes, and \(\Delta \phi_{x,y}^{s l}\) are the phase advances from the \(l^{\mathrm{th}}\) skew quadrupole to the observation point at the longitudinal coordinate \(s\).
...
As the studied interactions coming from these collisions are rare, a substantial interaction rate is required. 
...
The \concept{instantaneous luminosity} \(\mathcal{L}\) is the proportionality factor between the number of events per unit of time \(dR / dt\) and the process cross-section \(\sigma_{\mathrm{cross}}\):
