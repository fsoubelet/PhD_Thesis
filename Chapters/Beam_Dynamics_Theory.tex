...
In the \concept{uncoupled} case, this corresponds to a \(4 \times 4\) matrix with the respective \num{2}D transfer matrices on the diagonal and zeros elsewhere.
...
This leads to \concept{coupled motion} and \gls{betatron_coupling}, where the horizontal and vertical coordinates no longer evolve independently.
...
\begin{noteblock}
    ...
    However, for monochromatic beams with constant momentum the angular displacement (e.g. no acceleration) \(z^{\prime}\) is linked to the transverse momentum \(p_z\) by \(p_z = \beta_r \gamma_r m_0 c z^{\prime}\), with \(m_0\) the particle's rest mass and \(\beta_r, \gamma_r\) the relativistic factors.
...
For instance, the transfer matrix of a \gls{skew} quadrupole is:

\begin{equation}
    \mathrm{M_{skew\ quad.}} = \begin{pmatrix}
        M_x  & M_{xy}  \\
        M_{yx} & M_y   \\
    \end{pmatrix} \text{ ,}
